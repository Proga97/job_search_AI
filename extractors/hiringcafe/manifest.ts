...
function resolveDateFetchedPastNDays(context: HiringCafeContext): number {
	const postingAgeHours = Number(context.settings.jobspyHoursOld);
	if (!Number.isFinite(postingAgeHours) || postingAgeHours <= 0) return 30;
	return Math.max(1, Math.ceil(postingAgeHours / 24));
}
...
export const manifest: ExtractorManifest = {
	...
	async run(context) {
		...
		const result = await runHiringCafe({
			...
			maxJobsPerTerm,
			dateFetchedPastNDays: resolveDateFetchedPastNDays(context),
