...
function createRawJob(overrides: Record<string, unknown> = {}) {
	return {
		...
		job_information: {
			title: "Web Developer",
			description: "Build useful web things. Requires 2-5 years of experience.",
		},
...
describe("runHiringCafe", () => {
	...
	it("normalizes open-ended experience requirements from descriptions", async () => {
				...
				createSearchHtml([
					createRawJob({
						job_information: {
							...
							description: "You need at least 5 years of relevant experience.",
						},
					}),
				]),
			),
		...
		expect(result.jobs[0]?.experienceRange).toBe("5+ years");
	});
