...
Reactive Resume provides the structured base resume that Meow AI uses for PDF generation.

Meow AI uses a selected RxResume base resume as the source of truth, applies job-specific tailoring (summary, headline, skills, project visibility), then renders the final PDF using the renderer selected in Settings.
...
### Why Meow AI uses RxResume (instead of building a new editor)
...
- It has a JSON-native model (import/export), which is critical for Meow AI automation.

Because RxResume uses structured JSON, Meow AI can safely apply LLM-driven updates to specific sections before rendering PDFs locally.
...
Meow AI reads projects from `sections.projects.items` in the selected RxResume resume.
...
Meow AI caches successful Reactive Resume resume fetches in memory for 5 minutes.
