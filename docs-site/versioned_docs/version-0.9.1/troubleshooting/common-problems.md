...
- For `openai`, Meow AI defaults to `gpt-5.4-mini` when the model field is blank.
- For `codex`, Meow AI defaults to `gpt-5.4-mini` when the model field is blank.
- For `glm`, Meow AI defaults to `glm-5.1` and `https://api.z.ai/api/paas/v4` when the model/base URL fields are blank.
- For `gemini`, Meow AI defaults to `google/gemini-3-flash-preview` when the model field is blank.
