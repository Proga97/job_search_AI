...
description: Configure Reactive Resume as an optional import/export and PDF backend for Meow AI.
sidebar_position: 4
---

## What it is

Reactive Resume is now an optional integration in Meow AI.

The primary editing surface is **Design Resume**, which stores a local resume document inside Meow AI. Reactive Resume is used for:

- importing an existing resume into Design Resume
- exporting Meow AI resume JSON back out
...
Reactive Resume still matters because many users already maintain a strong resume there, and its JSON model is compatible with Meow AI. Importing that data into Design Resume avoids re-entry while reducing day-to-day dependence on the upstream service.
...
- Its JSON schema remains a stable interchange format for Meow AI.

For everyday editing inside Meow AI, use [Design Resume](./design-resume).
...
After import, Meow AI uses the local Design Resume for:
...
If you use the `rxresume` PDF renderer, Meow AI can still send the tailored JSON through Reactive Resume at export time.

### Project catalog

Meow AI reads projects from `sections.projects.items` in the local Design Resume document.
...
Before connecting Reactive Resume to Meow AI:
...
If you leave the URL blank in the dashboard, Meow AI uses `RXRESUME_URL` when it is set; if not set, it falls back to the public cloud default for the selected mode.

### Save-time validation

When you save Reactive Resume credentials or the shared URL in Settings:

1. Meow AI validates only the credential-bearing Reactive Resume fields for the selected mode.
...
3. Wait for Meow AI to create the local Design Resume document.

After that import, Meow AI reads resume context locally by default.
...
5. Normalize the tailored resume data into Meow AI' renderer document model.
...
Meow AI caches successful Reactive Resume resume fetches in memory for 5 minutes when upstream access is needed.
...
Meow AI can generate the final PDF in 2 ways:
...
- Project lists look stale:
  Re-import from Reactive Resume if you intentionally changed the upstream base resume and want those changes copied into Meow AI.
...
- Re-select/refresh the base resume in Meow AI and regenerate the PDF.
...
- Ensure `tectonic` is installed on the machine running Meow AI.
