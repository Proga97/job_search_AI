...
- upload a PDF or DOCX resume directly into Meow AI
...
3. Wait for Meow AI to create or replace the local Design Resume.
...
- There is no OCR or local text-extraction fallback. If the configured model cannot accept attached files, Meow AI returns an error instead.
...
- File import says the model cannot handle the file directly:
  Switch to a model with native document upload support, then try the import again. Meow AI does not fall back to OCR or local text extraction.
