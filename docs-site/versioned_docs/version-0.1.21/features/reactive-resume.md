...
Reactive Resume integration powers Meow AI PDF generation.

Meow AI uses a selected RxResume base resume as the source of truth, then applies job-specific tailoring (summary, headline, skills, project visibility) before exporting a PDF.
...
Because RxResume uses structured JSON, Meow AI can safely apply LLM-driven updates to specific sections before generating PDFs.
...
3. Generate/store that password so Meow AI can use it for API login.

Meow AI cannot use OAuth-based RxResume logins for this integration.
