...
The primary editing surface is **Resume Studio**, which stores a local resume document inside Meow AI. Reactive Resume is used for:
...
Reactive Resume still matters because many users already maintain a strong resume there, and its JSON model is compatible with Meow AI. Importing that data into Resume Studio avoids re-entry while reducing day-to-day dependence on the upstream service.
...
For everyday editing inside Meow AI, use [Resume Studio](./design-resume).
...
After import, Meow AI uses the local Resume Studio document for:
...
Meow AI reads projects from `sections.projects.items` in the local Resume Studio document.
...
If you leave the URL blank in the dashboard, Meow AI uses `RXRESUME_URL` when it is set; if not set, it falls back to the public cloud default (https://rxresu.me).

### Save-time validation

When you save Reactive Resume credentials or the shared URL in Settings:

1. Meow AI validates your API key.
...
3. Wait for Meow AI to create the local Resume Studio document.
...
Meow AI can generate the final PDF in 3 ways:
