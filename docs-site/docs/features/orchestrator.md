...
The Orchestrator is the primary jobs workspace in Meow AI.
...
- salary filters
- minimum suitability score filters (`50+`, `60+`, `70+`, `80+`, or `90+`); unscored jobs are excluded while active
- experience filters for internship, entry, mid, senior, and unlisted roles
- date filters
- sorting controls

### Hiding a job listing

Use the **Hide this listing** control on a job tile to remove an unwanted role
from the active workspace. Meow AI keeps the listing in a skipped state so its
source URL remains known. If a later search returns the same listing, it is
skipped instead of appearing again.

Use **Hide company** on a job tile to remove the company's current listings and
add its exact name to the hidden-company list. Future searches drop matching
listings during discovery, before they are stored. Edit the list under
**Settings → Display Preferences → Hidden companies**; after removing a company,
run Refresh to fetch its listings again.

### Auto-refreshing job searches

Use the clock toggle beside **Refresh** to enable or disable automatic searches.
Choose the cadence under **Settings → Display Preferences**: 30 minutes, 1
hour, 2 hours, 5 hours, 12 hours, or 24 hours. Auto-refresh uses the same saved
sources and search configuration as the manual Refresh action and runs while
the Jobs workspace is open.

Job rows and the detail header can also show a `Previously Applied` warning when Meow AI finds a high-confidence fuzzy match against one of your past applied or in-progress jobs using title and company, not just URL dedupe. To avoid flagging genuinely new openings, Meow AI only shows this warning when the matched historical application falls within 30 days of the current job's discovery date.
...
Date sorting follows the active date-filter context. If multiple date dimensions are enabled, Meow AI uses this priority:
...
If a job does not have the first selected timestamp, Meow AI falls back to the next available date in that order.
...
In the `ready` view, Meow AI can show prebuilt search links based on the current job's employer, title, and skills.
