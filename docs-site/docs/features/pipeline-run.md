...
The modal provides one place to describe a search, review the generated settings, and control run volume and source compatibility before fetching listings.
...
3. For Automatic, configure the search manually or switch to the AI setup.
4. Review the settings, edit anything you want, then run the search.
5. Search fetches, filters, deduplicates, and saves matching listings as **Discovered**. It does not run AI scoring or resume tailoring.
6. Open **Discovered** and select **Run AI scoring pipeline** when you are ready to score every unscored discovered job.
7. Select the jobs you want and use **Tailor selected** to generate their tailored resume content and move them to **Ready**.

This separation lets you inspect the raw search results before spending model usage on scoring or tailoring.

### AI scoring requests and Gemini limits

The scoring pipeline processes up to three unscored jobs in one model request.
Each response contains both the suitability score and the concise job brief, so
the app does not make a second brief-generation request for those jobs. Results
are saved after every batch; resuming a stopped run only selects jobs whose
score is still empty.

When the Gemini API provider is selected, all Gemini requests share a
project-wide queue. The default is 8 requests per minute and can be overridden
with `GEMINI_REQUESTS_PER_MINUTE`. Rate-limit and temporary upstream failures
retry with backoff. This limit also covers Gemini requests outside scoring so
parallel features cannot accidentally exceed the same Google project quota.

The header's **More job actions** menu also includes **Clear all job
listings**. It requires confirmation and deletes job listings plus their
job-linked notes, documents, and application history. Saved search settings and
pipeline run history remain available.

### Automatic tab

#### Describe search

The Automatic tab opens on **Configure manually** first.
...
Use **Date posted** to limit discovery to listings from the past 24 hours, 3
days, week, 2 weeks, or month. The default is **Past 3 days**. The pipeline
enforces the cutoff across every source before importing jobs; listings with an
older, missing, invalid, or future posting date are excluded. This setting is
remembered for Refresh and saved with saved searches.
...
- Country selection affects which sources are available, but the country list is owned by Meow AI rather than by any single extractor.
...
- **Jobs to prioritize** (`topN`) for legacy/full pipeline integrations
- **Min suitability score** used by scoring and legacy/full pipeline integrations
...
The footer estimate shows the expected number of discovered jobs. AI scoring and tailoring are started separately after discovery.
