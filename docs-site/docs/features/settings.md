...
- Select **Save configuration** below Default model to store the provider, base URL, API key, and model together. Saved configurations appear below the button and can be activated or deleted. The same provider with a different model is stored as a separate entry; API keys remain server-side and are shown only as “API key saved.”
...
- The settings page shows provider-aware model pickers for:
  ...
  - `gemini_cli`: a curated list of Gemini model ids the CLI typically supports (install [Gemini CLI](https://www.npmjs.com/package/@google/gemini-cli), run `gemini` and complete Google sign-in, or set `GEMINI_API_KEY` for the CLI; Meow AI spawns headless `gemini -p ...` with `--approval-mode plan` and no Meow AI API key field). **Resume import** uses the CLI with extracted text: DOCX is parsed locally; PDF uses local text extraction then JSON extraction via the CLI (scanned PDFs without a text layer may not import well).
  - `ollama`: locally installed Ollama models
- `openrouter`, `lmstudio`, and `openai_compatible` stay manual-entry because Meow AI cannot safely infer the exact model catalog from those providers
- For GLM, Meow AI uses `https://api.z.ai/api/paas/v4` by default. Override the base URL only when using another Z.AI-compatible endpoint such as a coding-plan endpoint.
...
- Automatic language detection currently supports English, German, French, and Spanish. If detection is unclear or the text is not in a supported language, Meow AI falls back to English.
...
4. Under the language control, choose one of these modes:
   ...
   - **Match Resume**: let Meow AI infer the language from your resume/profile text.
   - **Match job description**: let Meow AI infer the language from each job description.
...
- If Meow AI cannot determine a reliable supported language, it safely uses English.
...
- When Typst is selected, choose a Typst theme:
  - Classic
  - Compact
- Meow AI uses the selected RxResume resume as the source of truth for import and project data
...
- Meow AI briefly caches successful Reactive Resume resume data to reduce repeated API calls across settings, profile, and PDF flows
...
- a valid public Meow AI base URL
...
- Meow AI ignores stale Gemini-style overrides under `openai`, stale OpenAI-style overrides under `gemini`, and non-GLM-looking overrides under `glm`, but you still need to save the current form selection for future runs.
...
- If detection is ambiguous, Meow AI falls back to English by design.

### My headline or target job title did not get translated

- This is expected during resume tailoring.
- Meow AI intentionally preserves exact headline and job-title wording for ATS safety, even when other tailored sections are generated in another language.
...
- Meow AI resolves the RxResume URL in this order: the value saved in **Settings → Reactive Resume**, then the `RXRESUME_URL` environment variable (if set), and finally the public cloud default.
- Open **Settings → Reactive Resume** and configure the shared RxResume URL if you use a self-hosted instance.
- If you leave the URL blank, Meow AI will fall back to `RXRESUME_URL` when it is configured; otherwise it uses the public cloud default.
...
- Then return to Meow AI, refresh/select the same base resume in **Reactive Resume**, and regenerate the PDF.
- Meow AI preserves current visibility state, but it cannot create missing project content if the source resume has no projects.
