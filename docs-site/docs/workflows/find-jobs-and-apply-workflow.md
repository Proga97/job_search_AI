...
This guide documents the main intended pre-application workflow in Meow AI.
...
3. If you wrote a resume outside Meow AI, use **Upload PDF** in the job detail view to attach that file to the application instead of using the generated version.
...
### 5) Mark jobs as applied in Meow AI

After submitting, return to Meow AI and mark the job as `applied`.
You can mark either a `discovered` or `ready` job as applied; scoring, tailoring,
and PDF generation are optional.
