...
const JobsDefaultRedirect: React.FC = () => {
  const [destination, setDestination] = useState<string | null>(null);

  useEffect(() => {
    let active = true;
    api
      .getJobs({ statuses: ["ready", "processing"], view: "list" })
      .then((result) => {
        if (active) {
          setDestination(result.total > 0 ? "/jobs/ready" : "/jobs/discovered");
        }
      })
      .catch(() => {
        if (active) setDestination("/jobs/discovered");
      });
          ...
          <div className="mx-auto flex items-center justify-center gap-3">
            <p className="flex-1 text-center">
              ...
              {!demoWaitlistBannerDismissed && (
                <>
                  {" "}
                  This is a read-only demo. Want Meow AI without the Docker
                  setup? Demo data is temporary.
                </>
              )}
            </p>
            {!demoWaitlistBannerDismissed && (
          ...
          >
            <div ref={nodeRef}>
              <Routes location={location}>
                ...
                {/* Application routes */}
                <Route path="/" element={<JobsDefaultRedirect />} />
                <Route path="/jobs" element={<JobsDefaultRedirect />} />
