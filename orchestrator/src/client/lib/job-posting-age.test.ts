...
describe("formatPostingAgeLabel", () => {
	...
	it("keeps showing hours when a posting crosses midnight but is under 24 hours old", () => {
		expect(formatPostingAgeLabel("2026-05-26T13:00:00.000Z", NOW)).toEqual(
			expect.objectContaining({
				label: "23h ago",
				inlineLabel: "Posted 23h ago",
			}),
		);
	});

	it("uses a day count instead of yesterday after 24 hours", () => {
		expect(formatPostingAgeLabel("2026-05-26T11:00:00.000Z", NOW)).toEqual(
			expect.objectContaining({
				label: "1d ago",
				inlineLabel: "Posted 1d ago",
			}),
		);
	});
