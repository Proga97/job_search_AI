...
export const JobDescriptionPanel: React.FC<JobDescriptionPanelProps> = ({
	...
	const body = (
		<div
			className={cn(
				"w-full overflow-y-auto bg-card p-4 text-card-foreground",
				maxHeightClassName,
			)}
		>
			{isLoading ? (
				...
				<Textarea
					...
					className="min-h-[360px] bg-background font-mono text-sm leading-relaxed text-foreground focus-visible:ring-1"
	...
	if (!collapsible) {
		return (
			...
			>
				<div className="border-b border-border bg-muted px-3 py-3">
		...
		>
			<AccordionItem value="job-description" className="border-b-0">
				<div className="relative">
					<AccordionTrigger className="flex cursor-pointer items-center justify-between gap-2 border-b border-border bg-muted px-3 py-2.5 pr-4 text-left hover:bg-accent hover:no-underline">
						<div className="min-w-0">
							<div className="flex items-center gap-2 text-sm font-semibold text-foreground/90">
								<FileText className="h-3.5 w-3.5 text-sky-400/80" />
								Job description
