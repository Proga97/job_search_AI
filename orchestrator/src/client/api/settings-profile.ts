...
export type SavedLlmConfiguration = {
	id: string;
	provider: string;
	baseUrl: string | null;
	model: string;
...
export async function getSavedLlmConfigurations(): Promise<
	SavedLlmConfiguration[]
> {
	const data = await fetchApi<{ configurations: SavedLlmConfiguration[] }>(
...
export async function activateLlmConfiguration(
	...
	const data = await fetchApi<{ configuration: SavedLlmConfiguration }>(
		`/settings/llm-configurations/${encodeURIComponent(id)}/activate`,
		{ method: "POST" },
	);
	return data.configuration;
}

export async function deleteLlmConfiguration(
	id: string,
): Promise<SavedLlmConfiguration[]> {
	const data = await fetchApi<{ configurations: SavedLlmConfiguration[] }>(
		`/settings/llm-configurations/${encodeURIComponent(id)}`,
		{ method: "DELETE" },
	);
	return data.configurations;
}
