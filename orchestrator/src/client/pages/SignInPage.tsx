...
function resolveNextPath(rawNext: string | null): string {
  if (!rawNext || !rawNext.startsWith("/")) return "/jobs";
...
export function SignInPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (cancelled) return;
        setLicenseActive(license.active);
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!normalizedUsername || !password) {
      setErrorMessage("Enter both username and PIN.");
      return;
    }
    if (authMode === "signup" && !/^\d{4}$/.test(password)) {
  ...
  const description =
    authMode === "signup"
      ? "Create your Meow AI account for this hosted workspace."
      : "Enter your Meow AI username and PIN.";
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  inputMode={authMode === "signup" ? "numeric" : undefined}
                  pattern={authMode === "signup" ? "[0-9]{4}" : undefined}
                  minLength={authMode === "signup" ? 4 : undefined}
