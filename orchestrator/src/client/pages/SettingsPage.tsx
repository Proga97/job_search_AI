...
const DEFAULT_FORM_VALUES: UpdateSettingsInput = {
  ...
  autoRefreshJobsEnabled: null,
  autoRefreshJobsIntervalMinutes: 60,
  hiddenCompanies: [],
    ...
    items: [
      {
        ...
        description: "Job refresh, sponsor badges, and rendering behavior.",
        searchTerms: [
          "auto refresh",
          "interval",
          "jobs",
...
const NULL_SETTINGS_PAYLOAD: UpdateSettingsInput = {
  ...
  showSponsorInfo: null,
  autoRefreshJobsEnabled: null,
  autoRefreshJobsIntervalMinutes: null,
...
const mapSettingsToForm = (data: AppSettings): UpdateSettingsInput => ({
  ...
  autoRefreshJobsEnabled: data.autoRefreshJobsEnabled.override,
  autoRefreshJobsIntervalMinutes: (data.autoRefreshJobsIntervalMinutes
    .override ??
    data.autoRefreshJobsIntervalMinutes
      .value) as UpdateSettingsInput["autoRefreshJobsIntervalMinutes"],
  hiddenCompanies: data.hiddenCompanies.override ?? [],
  renderMarkdownInJobDescriptions:
  ...
  scoringRubric: data.scoringRubric.override ?? data.scoringRubric.value,
...
const getDerivedSettings = (settings: AppSettings | null) => {
  ...
  return {
    ...
    display: {
      ...
      autoRefreshJobsEnabled: {
        effective: settings?.autoRefreshJobsEnabled?.value ?? false,
        default: settings?.autoRefreshJobsEnabled?.default ?? false,
      },
      autoRefreshJobsIntervalMinutes: {
        effective: settings?.autoRefreshJobsIntervalMinutes?.value ?? 60,
        default: settings?.autoRefreshJobsIntervalMinutes?.default ?? 60,
      },
      hiddenCompanies: {
        effective: settings?.hiddenCompanies?.value ?? [],
        default: settings?.hiddenCompanies?.default ?? [],
      },
      renderMarkdownInJobDescriptions: {
    ...
    scoring: {
      ...
      scoringRubric: {
        effective: settings?.scoringRubric?.value ?? DEFAULT_SCORING_RUBRIC,
        default: settings?.scoringRubric?.default ?? DEFAULT_SCORING_RUBRIC,
      },
      autoSkipScoreThreshold: {
...
export const SettingsPage: React.FC = () => {
  ...
  const onSave = async (data: UpdateSettingsInput) => {
    ...
    try {
      ...
      const payload: Partial<UpdateSettingsInput> = {
        ...
        autoRefreshJobsEnabled: nullIfSame(
          data.autoRefreshJobsEnabled,
          display.autoRefreshJobsEnabled.default,
        ),
        autoRefreshJobsIntervalMinutes: nullIfSame(
          data.autoRefreshJobsIntervalMinutes,
          display.autoRefreshJobsIntervalMinutes.default,
        ) as UpdateSettingsInput["autoRefreshJobsIntervalMinutes"],
        hiddenCompanies: (() => {
          const normalized = normalizeStringArray(data.hiddenCompanies);
          const normalizedDefault = normalizeStringArray(
            display.hiddenCompanies.default,
          );
        ...
        scoringRubric:
          JSON.stringify(data.scoringRubric) ===
          JSON.stringify(scoring.scoringRubric.default)
            ? null
            : data.scoringRubric,
        ...
        if (!precheckFailure) {
          ...
          if (isRxResumeAvailabilityValidationFailure(validation)) {
            rxResumeSaveWarningMessage =
              "Settings saved, but Meow AI could not verify Reactive Resume because the instance is unavailable.";
          }
        }
      }
  ...
  return (
    <FormProvider {...methods}>
      ...
      <main className="container mx-auto px-4 py-6 pb-12 lg:max-w-[calc((100%+64rem)/2)] lg:px-1 xl:max-w-[calc((100%+80rem)/2)] 2xl:max-w-[calc((100%+96rem)/2)]">
