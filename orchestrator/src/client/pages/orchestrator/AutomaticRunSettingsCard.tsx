...
interface AutomaticRunSettingsCardProps {
	...
	onPostingAgeHoursChange: (value: number) => void;
}
			...
			<CardContent className="space-y-6 pt-5 sm:!pt-6">
				...
				<div className="grid items-center gap-3 md:grid-cols-[120px_1fr]">
					<div className="space-y-1">
						<Label htmlFor="posting-age" className="text-base font-semibold">
							Date posted
						</Label>
						<p className="text-xs text-muted-foreground">Only fetch recent jobs</p>
					</div>
					<Select
						value={String(postingAgeHours)}
						onValueChange={(value) => onPostingAgeHoursChange(Number(value))}
					>
						<SelectTrigger id="posting-age" className="h-10 w-full md:max-w-xs">
							<SelectValue />
						</SelectTrigger>
						<SelectContent>
