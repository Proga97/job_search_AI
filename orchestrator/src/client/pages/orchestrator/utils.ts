...
export const getAutomaticJobSort = (jobs: JobListItem[]): JobSort =>
	jobs.some((job) => job.suitabilityScore != null)
...
export const getExperienceLevel = (job: JobListItem): ExperienceLevel => {
	const text =
		`${job.experienceRange ?? ""} ${job.jobLevel ?? ""} ${job.title}`.toLowerCase();
	if (/\b(intern|internship)\b/.test(text)) return "internship";
	if (/\b(entry|entry-level|new grad|graduate|junior|jr\.?)\b/.test(text)) {
		return "entry";
	}
	if (/\b(senior|sr\.?|staff|principal|lead|director|manager)\b/.test(text)) {
		return "senior";
	}
	const years = [
		...text.matchAll(/(\d+)\s*(?:\+|[-–]\s*\d+)?\s*(?:years?|yrs?)/g),
	].map((match) => Number(match[1]));
	if (years.length > 0)
		return Math.min(...years) >= 6
			? "senior"
			: Math.min(...years) >= 2
				? "mid"
				: "entry";
	if (job.experienceRange?.trim() || job.jobLevel?.trim()) return "mid";
	return "not_listed";
};
...
) => selected.includes(getExperienceLevel(job));
