...
const renderFilters = (
	...
	const props = {
		...
		sponsorFilter: [] as SponsorFilter,
		...
		minimumScore: null,
		onMinimumScoreChange: vi.fn(),
		employmentTypes: [],
		onEmploymentTypesChange: vi.fn(),
		experienceLevels: [],
		onExperienceLevelsChange: vi.fn(),
		locationFilter: "",
...
describe("OrchestratorFilters", () => {
	...
	it("updates source, sponsor, and salary range from the filter bar", async () => {
		...
		expect(props.onSponsorFilterChange).toHaveBeenCalledWith([
			"sponsor_listed",
		]);
	...
	it("selects a minimum suitability score", async () => {
		const { props } = renderFilters();

		fireEvent.click(screen.getByRole("button", { name: /^score/i }));
		fireEvent.click(await screen.findByRole("button", { name: "80+" }));

		expect(props.onMinimumScoreChange).toHaveBeenCalledWith(80);
	});

	it("adds another sponsorship status without clearing the current one", async () => {
		const { props } = renderFilters({
			sponsorFilter: ["sponsor_listed"],
		});
		...
		fireEvent.click(screen.getByRole("button", { name: /^sponsor/i }));
		fireEvent.click(
			await screen.findByRole("button", { name: "Confirmed sponsor" }),
		);
