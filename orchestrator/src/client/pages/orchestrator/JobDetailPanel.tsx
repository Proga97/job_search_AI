...
> = {
  brief: {
    ...
    selectedClassName:
      "!border-sky-500/45 !bg-sky-100 !text-sky-900 dark:!border-sky-400/45 dark:!bg-sky-950 dark:!text-sky-100",
  },
  tailoring: {
    label: "Tailoring",
    ...
    selectedClassName:
      "!border-amber-500/45 !bg-amber-100 !text-amber-900 dark:!border-amber-400/45 dark:!bg-amber-950 dark:!text-amber-100",
  },
  apply: {
    label: "Apply",
    ...
    selectedClassName:
      "!border-emerald-500/45 !bg-emerald-100 !text-emerald-900 dark:!border-emerald-400/45 dark:!bg-emerald-950 dark:!text-emerald-100",
  },
};
...
const canMarkJobApplied = (job: Job): boolean =>
...
export const JobDetailPanel: React.FC<JobDetailPanelProps> = ({
  ...
  const handleMarkApplied = useCallback(async () => {
    if (!selectedJob || !canMarkJobApplied(selectedJob)) return;
    try {
      setIsApplying(true);
  ...
  const markAppliedIsPrimary =
    selectedJob.status === "ready" && (!hasJobListing || hasOpenedJobListing);
  const isApplied = selectedJob.status === "applied";
  ...
  return (
    ...
    >
      <TabsList className="mb-4 grid h-auto grid-cols-3 gap-1 rounded-lg border border-border bg-muted text-sm">
        {Object.entries(tabCopy).map(([value, copy]) => {
          ...
          const trigger = (
            <TabsTrigger
              ...
              value={value}
              className={cn(
                "flex flex-1 items-center gap-1.5 text-foreground hover:bg-accent hover:text-accent-foreground lg:flex-none",
      ...
      <JobHeader
        ...
        jobCTA={
          <div className="flex min-w-0 flex-wrap items-center gap-2 sm:shrink-0">
            {canMarkJobApplied(selectedJob) ? (
              <Button
                size="sm"
                variant="outline"
                onClick={() => void handleMarkApplied()}
                disabled={primaryBusy}
                className="border-border/70 bg-transparent text-foreground hover:bg-muted/80"
              >
                {isApplying ? (
                  <Loader2 className="h-3.5 w-3.5 animate-spin" />
                ) : (
              ...
              <Button
                size="sm"
                disabled
                className="border-emerald-500/40 bg-emerald-600 text-white disabled:opacity-100"
              >
                <CheckCircle2 className="h-3.5 w-3.5" />
                Applied
              </Button>
            ...
            {selectedJob.status !== "ready" ? (
              <Button
                ...
                className={cn("min-w-0 justify-center", tone.button)}
              >
                {primaryBusy ? (
                  <Loader2 className="h-3.5 w-3.5 animate-spin" />
            ...
            <GhostwriterDrawer
              ...
              triggerClassName="min-w-0 justify-start overflow-hidden"
            />
      ...
      <div className="flex min-w-0 flex-col rounded-lg rounded-t-none border border-t-0 border-border bg-card p-4 text-card-foreground">
        ...
        <TabsContent value="apply">
          <div className="space-y-5">
            ...
            <div className="grid gap-2 sm:grid-cols-3">
              ...
              <Button
                variant={
                  markAppliedIsPrimary || isApplied ? "default" : "outline"
                }
                className={cn(
                  (markAppliedIsPrimary || isApplied) &&
                    activeApplyCtaClassName,
                  isApplied && "disabled:opacity-100",
                )}
                size="sm"
                onClick={() => void handleMarkApplied()}
                disabled={!canMarkJobApplied(selectedJob) || primaryBusy}
              >
                {isApplying ? (
                ...
                {!isApplied && <KbdHint shortcut="a" className="ml-auto" />}
              </Button>
            </div>
