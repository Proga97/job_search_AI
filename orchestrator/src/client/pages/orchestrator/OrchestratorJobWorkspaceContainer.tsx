...
import { getAutomaticJobSort, getJobCounts, getSourcesWithJobs } from "./utils";
...
interface OrchestratorJobWorkspaceContainerProps {
	...
	onRunScoringPipeline: () => Promise<void>;
...
> = ({
	...
	ui,
	openRunMode,
	onRunScoringPipeline,
	onHideCompany,
}) => {
	const jobListHandleRef = useRef<VirtualListHandle | null>(null);
	const effectiveSort = useMemo(() => {
		if (filters.searchParams.has("sort")) return filters.sort;
		const tab = tabs.find((candidate) => candidate.id === navigation.activeTab);
		const jobsInActiveTab =
			tab && tab.statuses.length > 0
				? jobs.filter((job) => tab.statuses.includes(job.status))
				: jobs;
		return getAutomaticJobSort(jobsInActiveTab);
	}, [filters.searchParams, filters.sort, jobs, navigation.activeTab]);
	...
	const unscoredDiscoveredCount = useMemo(
		() =>
			jobs.filter(
				(job) => job.status === "discovered" && job.suitabilityScore == null,
			).length,
		[jobs],
	);
	...
	return (
		<>
			<div className={selectedJobIds.size > 0 ? "pb-24 lg:pb-0" : undefined}>
				{navigation.activeTab === "discovered" &&
					unscoredDiscoveredCount > 0 && (
						<div className="mb-4 flex flex-col gap-3 rounded-2xl border border-primary/20 bg-primary/[0.06] p-4 sm:flex-row sm:items-center sm:justify-between">
							<div>
								<p className="text-sm font-semibold text-foreground">
									{unscoredDiscoveredCount}{" "}
									{unscoredDiscoveredCount === 1 ? "job is" : "jobs are"} ready
									for AI scoring
								</p>
								<p className="mt-0.5 text-xs text-muted-foreground">
									Scores matches only. Tailoring stays a separate action.
								</p>
							</div>
							<Button
								type="button"
								...
								onClick={() => void onRunScoringPipeline()}
							>
				...
				<OrchestratorJobsWorkspace
					...
					onExperienceLevelsChange={filters.setExperienceLevels}
