...
describe("orchestrator utils", () => {
	it("defaults to score when scores exist and posted date otherwise", () => {
		expect(
			getAutomaticJobSort([
				createJob({ suitabilityScore: null }),
				createJob({ suitabilityScore: 82 }),
			]),
		).toEqual({ key: "score", direction: "desc" });
		expect(
			getAutomaticJobSort([createJob({ suitabilityScore: null })]),
		).toEqual({ key: "datePosted", direction: "desc" });
	});
	...
	it("hides jobindex from the US-focused source catalog", () => {
		expect(getEnabledSources(createAppSettings())).not.toContain("jobindex");
	});
	...
	it("hides naukri from the US-focused source catalog", () => {
		expect(getEnabledSources(createAppSettings())).not.toContain("naukri");
	});

	it("hides gradcracker from the US-focused source catalog", () => {
		expect(getEnabledSources(createAppSettings())).not.toContain("gradcracker");
	});
