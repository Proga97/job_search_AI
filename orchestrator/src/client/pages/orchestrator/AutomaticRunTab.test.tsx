...
describe("AutomaticRunTab", () => {
	...
	it("opens with manual configuration first", () => {
		render(
			<AutomaticRunTab
				open
		...
		expect(screen.getAllByText("Configure details").length).toBeGreaterThan(0);
		expect(
			screen.getByRole("button", { name: "Use AI to configure" }),
		).toBeInTheDocument();
	});
	...
	it("applies a saved pipeline search in one selection", async () => {
		...
		openConfigureDetails();
		fireEvent.change(
			screen.getAllByRole("combobox", { name: "Saved searches" })[0],
