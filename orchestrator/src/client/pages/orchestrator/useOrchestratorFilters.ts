...
const allowedSponsorFilters: SponsorFilterValue[] = [
  "available",
  "unavailable",
  "sponsor_listed",
  "unknown",
...
const parseExperienceLevels = (value: string | null): ExperienceLevel[] => {
  if (!value) return [];
  return experienceLevelValues.filter((level) =>
    value.split(",").includes(level),
  );
};
...
export const useOrchestratorFilters = () => {
  ...
  const sponsorFilter = useMemo((): SponsorFilter => {
    const raw = searchParams.get("sponsor");
    if (!raw) return [];
    const legacyMap: Record<string, SponsorFilterValue> = {
      confirmed: "sponsor_listed",
      potential: "sponsor_listed",
      not_found: "unknown",
    };
    return [
      ...new Set(raw.split(",").map((value) => legacyMap[value] ?? value)),
    ].filter((value): value is SponsorFilterValue =>
      allowedSponsorFilters.includes(value as SponsorFilterValue),
    );
  }, [searchParams]);
  ...
  const setSponsorFilter = useCallback(
    (value: SponsorFilter) => {
      setSearchParams(
        (prev) => {
          if (value.length === 0) prev.delete("sponsor");
          else prev.set("sponsor", value.join(","));
          return prev;
        },
        { replace: true },
      );
  ...
  const postedWithinDays = useMemo((): number | null => {
    ...
    return postedWithinValues.includes(parsed) ? parsed : 1;
  }, [searchParams]);
  ...
  const setPostedWithinDays = useCallback(
    (value: number | null) => {
      setSearchParams(
        (prev) => {
          if (value == null) prev.set("postedWithin", "all");
  ...
  const experienceLevels = useMemo(
    () => parseExperienceLevels(searchParams.get("experience")),
    [searchParams],
  );
  const setExperienceLevels = useCallback(
    (values: ExperienceLevel[]) => {
      setSearchParams(
        (prev) => {
          if (values.length === 0) prev.delete("experience");
          else prev.set("experience", values.join(","));
          return prev;
        },
        { replace: true },
      );
  ...
  const minimumScoreRaw = Number(searchParams.get("minScore"));
  const minimumScore = scoreThresholdOptions.includes(
    minimumScoreRaw as (typeof scoreThresholdOptions)[number],
  )
    ? minimumScoreRaw
    : null;
