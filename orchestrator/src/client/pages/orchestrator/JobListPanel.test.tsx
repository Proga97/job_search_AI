...
describe("JobListPanel", () => {
	...
	it("visually stacks consecutive roles from the same company", () => {
		const jobs = [
			createJob({ id: "job-1", title: "Engineer I", employer: "X Corp" }),
			createJob({ id: "job-2", title: "Engineer II", employer: "X Corp" }),
			createJob({ id: "job-3", title: "Engineer III", employer: "X Corp" }),
		];
		...
		expect(screen.getAllByText("X Corp")).toHaveLength(3);
	});

	it("offers a persistent hide action for a job tile", async () => {
		const jobs = [createJob({ id: "job-1", title: "Hidden Role" })];
		const onHideJob = vi.fn().mockResolvedValue(undefined);
		...
		fireEvent.click(screen.getByRole("button", { name: "Hide Hidden Role" }));
		await waitFor(() => expect(onHideJob).toHaveBeenCalledWith(jobs[0]));
	});

	it("offers a company-level hide action for a job tile", async () => {
		const jobs = [createJob({ id: "job-1", employer: "Acme Labs" })];
		const onHideCompany = vi.fn().mockResolvedValue(undefined);
		...
		fireEvent.click(
			screen.getByRole("button", { name: "Hide company Acme Labs" }),
		);
		await waitFor(() => expect(onHideCompany).toHaveBeenCalledWith(jobs[0]));
	});
