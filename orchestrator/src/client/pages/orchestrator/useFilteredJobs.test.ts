...
type FilterableJob = Job & Partial<Pick<JobListItem, "visaSponsorship">>;
...
describe("useFilteredJobs", () => {
	it("filters out unscored jobs and scores below the selected threshold", () => {
		const jobs = [
			createJob({ id: "high", status: "ready", suitabilityScore: 82 }),
			createJob({ id: "low", status: "ready", suitabilityScore: 69 }),
			createJob({ id: "none", status: "ready", suitabilityScore: null }),
		];
		const { result } = renderHook(() =>
			useFilteredJobs(jobs, makeFilters({ minimumScore: 70 })),
		);

		expect(result.current.map((job) => job.id)).toEqual(["high"]);
	});

	it("filters jobs by inferred experience level", () => {
		const jobs = [
			createJob({ id: "entry", status: "ready", title: "Junior Engineer" }),
			createJob({ id: "senior", status: "ready", title: "Senior Engineer" }),
		];
		const { result } = renderHook(() =>
			useFilteredJobs(jobs, makeFilters({ experienceLevels: ["senior"] })),
		);

		expect(result.current.map((job) => job.id)).toEqual(["senior"]);
	});
		...
		const jobs: FilterableJob[] = [
			{
				...
				visaSponsorship: "sponsor_listed" as const,
