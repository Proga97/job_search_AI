...
describe("JobDetailPanel", () => {
  ...
  it("marks a discovered job as applied without scoring or tailoring", async () => {
    const onJobUpdated = vi.fn().mockResolvedValue(undefined);
    vi.mocked(api.markAsApplied).mockResolvedValue(undefined as any);
    ...
    const applyLink = within(header).getByRole("link", { name: "Apply" });
    ...
    expect(within(header).queryByText(/autofill/i)).not.toBeInTheDocument();
    ...
    expect(header.textContent?.indexOf("Mark Applied")).toBeLessThan(
      header.textContent?.indexOf("Start Tailoring") ?? -1,
    );
    expect(header.textContent?.indexOf("Apply")).toBeLessThan(
      header.textContent?.indexOf("Mark Applied") ?? -1,
    );
  ...
  it("moves an applied job to in progress from the action button", async () => {
    ...
    const appliedState = within(screen.getByTestId("job-header")).getByRole(
      "button",
      { name: "Applied" },
    );
    expect(appliedState).toHaveClass("bg-emerald-600");
