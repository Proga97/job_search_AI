...
async function insertJob(input: CreateJobInput): Promise<Job> {
	...
	await db.insert(jobs).values({
		...
		sponsorMatchScore: input.sponsorMatchScore ?? null,
		sponsorMatchNames: input.sponsorMatchNames ?? null,
		status: "discovered",
		discoveredAt: now,
		createdAt: now,
		updatedAt: now,
...
/** Delete every job belonging to the active tenant. */
export async function deleteAllJobs(): Promise<number> {
	const result = await db.delete(jobs).where(jobsScopeFilter()).run();
	return result.changes;
}
