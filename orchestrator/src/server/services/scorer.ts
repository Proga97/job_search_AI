...
import type { Job, JobBrief, ScoringRubric } from "@shared/types";
import { normalizeJobBrief } from "./job-brief";
...
export interface ScoringWithBriefResult extends SuitabilityResult {
  jobId: string;
...
const BATCH_SCORING_SCHEMA: JsonSchemaDefinition = {
  name: "job_scores_and_briefs",
  schema: {
    type: "object",
    properties: {
      results: {
        ...
        items: {
          ...
          properties: {
            ...
            brief: {
              ...
              properties: {
                ...
                they_want: { type: "array", items: { type: "string" } },
          ...
          required: ["job_id", "score", "reason", "brief"],
        },
      },
    },
    required: ["results"],
...
function formatScoringRubric(rubric: ScoringRubric): string {
  return [
    `- Skills match (technologies, frameworks, languages): 0-${rubric.skills} points`,
    `- Experience level match: 0-${rubric.experience} points`,
    `- Location/remote work alignment: 0-${rubric.location} points`,
    `- Industry/domain fit: 0-${rubric.domain} points`,
    `- Career growth potential: 0-${rubric.growth} points`,
  ].join("\n");
}

type ProfileRecord = Record<string, unknown>;
const DEFAULT_SCORING_RUBRIC: ScoringRubric = {
  skills: 30,
  experience: 25,
  location: 15,
  domain: 15,
...
export async function scoreJobSuitability(
  ...
  const prompt = buildScoringPrompt(job, sanitizeProfileForPrompt(profile), {
    ...
    rubric: settings.scoringRubric?.value ?? DEFAULT_SCORING_RUBRIC,
    promptTemplate:
...
/** Score up to a small batch of jobs and extract their briefs in one LLM call. */
export async function scoreJobsWithBriefs(
  jobs: Job[],
  profile: Record<string, unknown>,
  options: { scoringInstructions?: string } = {},
): Promise<ScoringWithBriefResult[]> {
  ...
  const sanitizedProfile = sanitizeProfileForPrompt(profile);
  const jobPrompts = jobs
    .map(
      (job) =>
        `JOB ID: ${job.id}\n${buildScoringPrompt(job, sanitizedProfile, preferences)}`,
    )
    .join("\n\n--- NEXT JOB ---\n\n");
  const prompt = `${jobPrompts}\n\nFor every job, return one result with the exact JOB ID, suitability score and reason. Also extract a neutral brief using only that job description. Brief arrays must be concise: they_want max 6, specifics max 18, company_offers max 5, practical_details max 8, missing_or_unclear max 5, repeated_signals max 5. Do not omit any job.`;

  const llm = await createConfiguredLlmService("scoring");
  const result = await llm.callJson<BatchLlmResult>({
    model,
    messages: [{ role: "user", content: prompt }],
    jsonSchema: BATCH_SCORING_SCHEMA,
    maxRetries: 4,
    retryDelayMs: 2_000,
  ...
  if (!result.success) {
    throw new LlmNotConfiguredError(
      `AI scoring failed: ${result.error}. The completed batches are saved; resume scoring after checking Settings → Integrations.`,
    );
  }

  const byId = new Map(result.data.results.map((item) => [item.job_id, item]));
  return jobs.map((job) => {
    const item = byId.get(job.id);
    if (!item || typeof item.score !== "number") {
      throw new LlmNotConfiguredError(
        `AI omitted ${job.title} from its batch response. Resume scoring to retry this batch.`,
      );
    }
    const score = Math.min(100, Math.max(0, Math.round(item.score)));
    const penalty = applySalaryPenalty(
      job,
      score,
    ...
    const brief = normalizeJobBrief(item.brief);
    return {
      jobId: job.id,
      score: penalty.score,
      reason: penalty.reason,
      jobBrief: brief ? JSON.stringify(brief) : null,
    };
  });
}
...
function buildScoringPrompt(
  ...
  const rubricText = formatScoringRubric(preferences.rubric);
  ...
  return preferences.promptTemplate.includes("{{scoringRubricText}}")
    ? prompt
    : `${prompt}\n\nSCORING RUBRIC (use these current Settings weights):\n${rubricText}`;
}
