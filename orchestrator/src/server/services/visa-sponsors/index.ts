...
export function resolveEmployerAlias(
  ...
  return (
    Object.entries(aliases ?? {}).find(
      ([brandName]) => normalizeCompanyName(brandName) === normalizedQuery,
    )?.[1] ?? query
  );
}
...
> {
  ...
  return manifests.map((m) => ({
    ...
    employerAliases: m.employerAliases,
  }));
}
...
export async function searchSponsors(
  ...
  for (const {
    ...
    const providerQuery = normalizeCompanyName(
      resolveEmployerAlias(query, employerAliases),
    );
    ...
    for (const sponsor of sponsors) {
      ...
      const score = calculateSimilarity(providerQuery, normalizedSponsor);
