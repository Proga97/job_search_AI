...
describe("visa sponsor provider discovery", () => {
	it("finds provider manifests in the repo-local providers directory", async () => {
		...
		await mkdir(join(providersRoot, "us"), { recursive: true });
		await writeFile(
			join(providersRoot, "us", "manifest.ts"),
			[
	...
	it("loads provider manifests from named exports", async () => {
		...
		expect(manifest.id).toBe("us");
		expect(manifest.countryKey).toBe("united states");
	});
