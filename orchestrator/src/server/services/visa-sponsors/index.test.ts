...
import { calculateSponsorMatchSummary, resolveEmployerAlias } from "./index";

describe("resolveEmployerAlias", () => {
	it("maps an exact normalized brand name to its legal sponsor entity", () => {
		expect(
			resolveEmployerAlias("HCLTech", {
				HCLTech: "HCL AMERICA INC",
			}),
		).toBe("HCL AMERICA INC");
	});

	it("does not apply an alias to a merely similar company name", () => {
		expect(
			resolveEmployerAlias("YouTube Music Services", {
				YouTube: "Google LLC",
			}),
		).toBe("YouTube Music Services");
	});
});
