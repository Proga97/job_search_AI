...
describe("scoreJobsStep auto-skip behavior", () => {
	beforeEach(async () => {
		...
		vi.mocked(scorer.scoreJobsWithBriefs).mockResolvedValue([
			{
				jobId: "job-1",
				score: 40,
				reason: "Low fit",
	...
	it("passes per-run scoring instructions to the scorer", async () => {
		...
		expect(scorer.scoreJobsWithBriefs).toHaveBeenCalledWith(
	...
	it("does not override global scoring instructions with a blank per-run value", async () => {
		...
		expect(vi.mocked(scorer.scoreJobsWithBriefs).mock.calls[0]).toHaveLength(2);
	});
	...
	it("scores multiple jobs and reports completion progress", async () => {
		...
		vi.mocked(scorer.scoreJobsWithBriefs).mockResolvedValueOnce([
			{ jobId: "job-1", score: 61, reason: "First score", jobBrief: null },
			{ jobId: "job-2", score: 72, reason: "Second score", jobBrief: null },
		]);
	...
	it("stops before processing when cancellation is requested", async () => {
		...
		expect(vi.mocked(scorer.scoreJobsWithBriefs)).not.toHaveBeenCalled();
