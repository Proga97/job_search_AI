...
export function isJobWithinPostingAge(
	...
	if (
		!datePosted ||
		!Number.isFinite(postingAgeHours) ||
		postingAgeHours <= 0
	) {
		return false;
	}
	const postedAtMs = Date.parse(datePosted);
	if (!Number.isFinite(postedAtMs) || postedAtMs > nowMs) return false;
	return postedAtMs >= nowMs - postingAgeHours * 60 * 60 * 1000;
}
...
export async function discoverJobsStep(args: {
	...
	const runSettings = {
		...budgetSettings,
		...(args.mergedConfig.postingAgeHours !== undefined
			? { jobspyHoursOld: String(args.mergedConfig.postingAgeHours) }
			: {}),
	};
	...
	return withHostedUsageReservation(
		...
		async () => {
			...
			const postingAgeFilteredJobs =
				args.mergedConfig.postingAgeHours === undefined
					? locationFilteredJobs
					: locationFilteredJobs.filter((job) =>
							isJobWithinPostingAge(
								job.datePosted,
								args.mergedConfig.postingAgeHours as number,
							),
						);
			const postingAgeFilteredOutCount =
				locationFilteredJobs.length - postingAgeFilteredJobs.length;

			if (postingAgeFilteredOutCount > 0) {
				logger.info("Dropped jobs outside the requested posting-date window", {
					step: "discover-jobs",
					postingAgeHours: args.mergedConfig.postingAgeHours,
					droppedCount: postingAgeFilteredOutCount,
				});
			}
			...
			const hiddenCompanies = parseBlockedCompanyKeywords(
				settings.hiddenCompanies,
			);
			const hiddenCompanyNames = new Set(
				hiddenCompanies.map(normalizeEmployerName),
			);
			const blockedKeywordsLowerCase = blockedCompanyKeywords.map((value) =>
				value.toLowerCase(),
			);
			const filteredDiscoveredJobs = postingAgeFilteredJobs.filter((job) => {
				if (isBlockedEmployer(job.employer, blockedKeywordsLowerCase)) {
					return false;
				}
				return !(
					job.employer &&
					hiddenCompanyNames.has(normalizeEmployerName(job.employer))
				);
			});
			const droppedCount =
				postingAgeFilteredJobs.length - filteredDiscoveredJobs.length;
