...
import { discoverJobsStep, isJobWithinPostingAge } from "./discover-jobs";
...
describe("isJobWithinPostingAge", () => {
	const now = Date.parse("2026-07-16T12:00:00.000Z");

	it("keeps only valid dates inside the selected window", () => {
		expect(isJobWithinPostingAge("2026-07-16T00:00:00.000Z", 24, now)).toBe(
			true,
		);
		expect(isJobWithinPostingAge("2026-07-14T12:00:00.000Z", 24, now)).toBe(
			false,
		);
	});

	it("rejects missing, invalid, and future posting dates", () => {
		expect(isJobWithinPostingAge(undefined, 24, now)).toBe(false);
		expect(isJobWithinPostingAge("not-a-date", 24, now)).toBe(false);
		expect(isJobWithinPostingAge("2026-07-17T12:00:00.000Z", 24, now)).toBe(
			false,
		);
	});
});
...
describe("discoverJobsStep", () => {
	...
	it("drops exact hidden company names before jobs are persisted", async () => {
		const settingsRepo = await import("@server/repositories/settings");
		const registryModule = await import("@server/extractors/registry");
		const jobspyManifest = {
			id: "jobspy",
				...
				jobs: [
					{
						...
						title: "Engineer",
						employer: "  Acme   Labs ",
						jobUrl: "https://example.com/hidden",
					},
					{
						source: "linkedin",
						title: "Engineer",
						employer: "Acme Labs Research",
						jobUrl: "https://example.com/allowed",
					},
				],
			}),
		};
		vi.mocked(settingsRepo.getAllSettings).mockResolvedValue({
			searchTerms: JSON.stringify(["engineer"]),
			hiddenCompanies: JSON.stringify(["ACME LABS"]),
		} as any);
		vi.mocked(registryModule.getExtractorRegistry).mockResolvedValue({
			manifests: new Map([["jobspy", jobspyManifest as any]]),
			manifestBySource: new Map([["linkedin", jobspyManifest as any]]),
