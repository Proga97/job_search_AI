...
const DEFAULT_CONFIG: PipelineConfig = {
	...
	sources: ["indeed", "linkedin", "hiringcafe", "startupjobs"],
...
export async function runPipeline(
	...
	return runWithRequestContext({ pipelineRunId: pipelineRun.id }, async () => {
		...
		try {
			...
			const profile =
				mergedConfig.enableScoring !== false ||
			...
			let discoveredJobs: CreateJobInput[] = [];
			...
			if (mergedConfig.enableCrawling !== false) {
				...
				await persistResultSummary({ stage: "discovery", sourceErrors });
			}
			...
			const importResult =
				mergedConfig.enableImporting !== false
					? await importJobsStep({
							discoveredJobs,
							visaSponsorCountryKey:
								mergedConfig.locationIntent?.selectedCountry,
						})
					: { created: 0, skipped: 0, fuzzyMerged: 0 };
			const { created, skipped, fuzzyMerged } = importResult;
			if (mergedConfig.enableImporting !== false) {
			...
			if (mergedConfig.enableScoring !== false) {
				if (!profile) throw new Error("A profile is required for AI scoring");
				ensureNotCancelled(scopeKey);
			...
			if (mergedConfig.enableAutoTailoring === false) {
				...
				progressHelpers.complete(jobsDiscovered, 0);
				pipelineLogger.info("Pipeline stage completed", {
					jobsDiscovered,
				...
				pipelineUsageUnits = mergedConfig.enableScoring === false ? 0 : 1;
				return { success: true, jobsDiscovered, jobsProcessed: 0 };
			}
