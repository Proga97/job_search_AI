...
jobsMaintenanceRouter.delete("/", async (_req: Request, res: Response) => {
  try {
    if (isDemoMode()) {
      return sendDemoBlocked(
        res,
        "Clearing all job listings is disabled to keep the demo stable.",
        { route: "DELETE /api/jobs" },
      );
    }

    const count = await jobsRepo.deleteAllJobs();
    ok(res, { message: `Cleared ${count} job listings`, count });
  } catch (error) {
