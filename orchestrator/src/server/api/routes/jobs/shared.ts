...
export function toJobListItem(
	...
	return {
		...
		experienceRange:
			job.experienceRange ?? inferExperienceRange(job.jobDescription),
		workFromHomeType: job.workFromHomeType,
		visaSponsorship: inferVisaSponsorship(
			job.jobDescription,
			job.sponsorMatchScore,
		),
...
export function inferVisaSponsorship(
	...
	if (descriptionText) {
		const unavailablePattern =
			/\b(?:will not|does not|do not|cannot|can't|unable to)\s+(?:provide\s+|offer\s+)?(?:visa\s+|employment\s+)?sponsor(?:ship)?|\bno\s+(?:visa\s+|employment\s+)?sponsorship\b|\bwithout\s+(?:the\s+)?need\s+for\s+(?:current\s+or\s+future\s+)?(?:visa\s+|employment\s+)?sponsorship\b|\b(?:work|employment)\s+authorization\s+that\s+does\s+not\s+(?:now\s+or\s+in\s+the\s+future\s+)?require\s+(?:visa\s+|employment\s+)?sponsorship\b/i;
		if (unavailablePattern.test(descriptionText)) return "unavailable";

		const availablePattern =
			/\b(?:visa\s+|employment\s+)?sponsorship\s+(?:is\s+)?(?:available|provided|offered)\b|\b(?:provide|offer|support)\w*\s+(?:.{0,40}\s+)?(?:visa\s+|employment\s+)?sponsorship\b|\beligible\s+for\s+(?:visa\s+|employment\s+)?sponsorship\b|\bsponsor\w*\s+(?:an?\s+)?h-?1b\b/i;
		if (availablePattern.test(descriptionText)) return "available";
	}

	if (sponsorMatchScore != null && sponsorMatchScore >= 80) {
		return "sponsor_listed";
	}

	return "unknown";
}

function inferExperienceRange(description: string | null): string | undefined {
	const descriptionText = description
		?.replace(/<[^>]+>/g, " ")
		.replace(/&nbsp;|&#160;/gi, " ")
		.replace(/\s+/g, " ")
	...
	const rangeMatch = descriptionText.match(
		/\b(\d{1,2})\s*(?:-|–|—|to)\s*(\d{1,2})\s*(?:years?|yrs?)\b/i,
	);
	if (rangeMatch) return `${rangeMatch[1]}-${rangeMatch[2]} years`;

	const plusMatch = descriptionText.match(
		/\b(\d{1,2})\s*\+\s*(?:years?|yrs?)\b/i,
	);
	if (plusMatch) return `${plusMatch[1]}+ years`;

	const minimumMatch = descriptionText.match(
		/\b(?:at least|minimum(?: of)?|minimum requirement(?: of)?|requires?)\s+(\d{1,2})\s*(?:years?|yrs?)\b/i,
	);
	if (minimumMatch) return `${minimumMatch[1]}+ years`;

	const explicitMatch = descriptionText.match(
		/\b(\d{1,2})\s*(?:years?|yrs?)\s+of\s+(?:professional\s+|relevant\s+|industry\s+|work\s+)?experience\b/i,
	);
