...
describe.sequential("Visa sponsors API routes", () => {
	...
	it("updates an individual provider and returns its refreshed status", async () => {
			...
			providers: [
				{
					...
					csvPath: "/tmp/us/visa_sponsors_2026-03-09.csv",
					totalSponsors: 123,
					isUpdating: false,
		...
		const res = await fetch(`${baseUrl}/api/visa-sponsors/update/us`, {
			method: "POST",
			headers: { "x-request-id": "req-visa-sponsors-us" },
		});
		const body = await res.json();

		expect(res.status).toBe(200);
		expect(res.headers.get("x-request-id")).toBe("req-visa-sponsors-us");
		expect(vi.mocked(downloadLatestCsv)).toHaveBeenCalledWith("us");
		expect(body.ok).toBe(true);
		expect(body.data.message).toBe("Updated 1/1 providers");
		expect(body.data.status.providers).toHaveLength(1);
		expect(body.meta.requestId).toBe("req-visa-sponsors-us");
	});
	...
	it("validates search payloads and handles missing organizations", async () => {
		...
		const orgRes = await fetch(
			`${baseUrl}/api/visa-sponsors/organization/Acme?providerId=us`,
		);
