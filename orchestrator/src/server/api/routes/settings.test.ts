...
describe.sequential("Settings API routes", () => {
	...
	it("allows saving when Reactive Resume is temporarily unavailable", async () => {
		vi.mocked(validateCredentials).mockResolvedValue({
			...
			status: 0,
			message:
				"Meow AI could not verify Reactive Resume because the instance is unavailable.",
		});
	...
	it("saves, activates, lists, and deletes LLM configurations without exposing API keys", async () => {
		const saveResponse = await fetch(
			`${baseUrl}/api/settings/llm-configurations`,
			{
				method: "POST",
				headers: { "content-type": "application/json" },
				body: JSON.stringify({
		...
		expect(saveBody.data.configuration).toMatchObject({
			provider: "gemini",
			model: "gemini-2.5-flash",
			hasApiKey: true,
		});
		expect(JSON.stringify(saveBody)).not.toContain("super-secret-key");

		const id = saveBody.data.configuration.id as string;
		const listResponse = await fetch(
			`${baseUrl}/api/settings/llm-configurations`,
		);
		const listBody = await listResponse.json();
		expect(listBody.data.configurations).toHaveLength(1);
		expect(JSON.stringify(listBody)).not.toContain("super-secret-key");

		const activateResponse = await fetch(
			`${baseUrl}/api/settings/llm-configurations/${id}/activate`,
			{ method: "POST" },
		);
		...
		expect(settingsBody.data.llmProvider.value).toBe("gemini");
		expect(settingsBody.data.model.value).toBe("gemini-2.5-flash");

		const deleteResponse = await fetch(
			`${baseUrl}/api/settings/llm-configurations/${id}`,
			{ method: "DELETE" },
		);
		const deleteBody = await deleteResponse.json();
		expect(deleteResponse.status).toBe(200);
		expect(deleteBody.data.configurations).toEqual([]);
	});
