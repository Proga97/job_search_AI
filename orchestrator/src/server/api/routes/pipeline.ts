...
const pipelineSearchPresetConfigSchema = z.object({
	...
	postingAgeHours: z.number().int().min(1).max(8760).optional(),
...
const runPipelineSchema = z.object({
	mode: z.enum(["discover", "score", "full"]).optional(),
...
pipelineRouter.post("/run", async (req: Request, res: Response) => {
	try {
		...
		const searchTermsState =
			config.mode === "score"
				? { searchTermsCount: 0, source: "not_applicable" as const }
		...
		ok(res, {
			message:
				config.mode === "score" ? "AI scoring started" : "Search started",
		});
	} catch (error) {
		if (error instanceof z.ZodError) {
			return fail(res, badRequest(error.message, error.flatten()));
