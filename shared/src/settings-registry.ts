  ...
  .refine(
    (value) =>
      value.skills +
        ...
        value.growth ===
      100,
    { message: "Scoring rubric weights must total 100" },
  );

const defaultScoringRubric = (): ScoringRubric => ({
  skills: 30,
  experience: 25,
  location: 15,
  domain: 15,
...
function parseScoringRubric(raw: string | undefined): ScoringRubric | null {
  const parsed = parseJsonObjectOrNull<unknown>(raw);
  const result = scoringRubricSchema.safeParse(parsed);
...
export const settingsRegistry = {
  ...
  hiddenCompanies: {
    kind: "typed" as const,
    schema: z.array(z.string().trim().min(1).max(500)).max(500),
    default: (): string[] => [],
    parse: parseJsonArrayOrNull,
    serialize: serializeNullableJsonArray,
  },
  ...
  scoringRubric: {
    kind: "typed" as const,
    schema: scoringRubricSchema,
    default: defaultScoringRubric,
    parse: parseScoringRubric,
    serialize: (value: ScoringRubric | null | undefined): string | null =>
